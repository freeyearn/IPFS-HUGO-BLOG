<template>
  <el-card>
    <div class="container">
        ...
        <el-page-header @back="$router.push('/articles')">
          <template #content>
            <el-text class="mx-1" size="large">{{
              $t("article.addArticleContent")
            }}</el-text>
          </template>
        </el-page-header>
        <div class="edit-size" style="border: 1px solid #ccc; margin-top: 15px" v-loading="loading.vditorLoading">
          <div id="vditor"></div>
        </div>
      </div>
      ...
      <div class="right">
        <el-text class="mx-1" size="large">{{
          $t("article.addArticleParameters")
        }}</el-text>
        <hr />
        <div class="form">
          <el-form ref="ruleFormRef" :rules="rules" :label-position="labelPosition" label-width="100px"
            ...
            <el-form-item :label="addArticleParameters.title" prop="title">
              <el-input v-model="formLabelAlign.title" placeholder="Please input blog title" clearable />
            </el-form-item>

            <el-form-item :label="addArticleParameters.description" prop="description">
              <el-input v-model="formLabelAlign.description" placeholder="Please input summary" autosize type="textarea"
                clearable />
            </el-form-item>
        ...
        <div class="btns">
          <el-button type="primary" style="float: right" @click="saveFun(ruleFormRef)" :loading="loading.saveLoading" >{{
            $t("Btns.save")
          }}</el-button>
        </div>
      </div>
...
const saveInfo = (status: number) => {
  const category = [];
  category[0] = formLabelAlign.category;
  const parms = {
